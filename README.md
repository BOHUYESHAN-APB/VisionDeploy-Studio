# 想法很好现现实却不好（25-11-09）
大多数模型需要一并克隆hf上的模型和GitHub上的代码才能运行，相比较于大语言模型使用llama.cpp一类的统一推理架构，视觉模型反而就十分杂乱，没有一统天下的趋势。开发难度较大，这个仓库还不如多做几个分支作为某某模型的一键包发布，后续会采取这个方式做一键包，让使用者快速使用。仓库内模型协议遵循原始模型协议，一键包协议会更具使用的代码每个分支及其局部采用对应协议，这个分支变为old。除非后续出现类似llama.cpp一类的架构出现否则不在启用。
