...
Run in VS Code using "Run Python File" to observe stdout/stderr.
"""
...
def main():
    ...
    with dpg.window(label="中文测试", width=480, height=220):
        dpg.add_text("测试中文显示: 你好，世界")  # static text
        dpg.add_spacing()
        ...
        # Note: some DPG versions require set_item_callback after creation
        dpg.set_item_callback("input_text", on_input)

        # transcode button to attempt iso-8859-1 -> gbk trick
    ...
    # Try rebind after viewport setup (some versions need this)
    try:
